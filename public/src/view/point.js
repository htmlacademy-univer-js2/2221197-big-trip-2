...
import { pointDueDate, duration, getDate, getTime } from '../util';
...
const createPointTemplate = (point, destinations, offers) => {
  const {basePrice, type, destinationID, isFavorite, dateFrom, dateTo, offerID} = point;
  const allPointTypeOffers = offers.find((offer) => offer.type === type);
  const eventDuration = duration(dateFrom, dateTo);
  const startDate = dateFrom !== null ? pointDueDate(dateFrom): '';
  const endDate = dateTo !== null ? pointDueDate(dateTo): '';
  ...
  return (
    `<li class="trip-events__item">
      ...
      <ul class="event__selected-offers">
        ${renderOffers(allPointTypeOffers.offers, offerID)}
      </ul>
...
export default class PointView {
  ...
  getTemplate() {
    return createPointTemplate(this.points, this.destination, this.offers);
  }
